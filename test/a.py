    ...
    def run_model_vin_group(vin_df):
        ...
        try:
            ...
            for group in grouped_df:
                ...
                if not m_model:
                    m_model = 'null'
                m_year = pd.to_numeric(vin_df['year'].iloc[0], errors='coerce')

                if np.isnan(m_year):
                    m_year = 1900                   
                m_date = vin_df['date'].iloc[0] 
                if not m_date:
                    m_date = '2020-01-01'
                m_flg_date = []    
                m_flg = [] 
                ...
                for day in day_list:
                    ...
                    if len(df_tmp[df_tmp['hitcount1']==0]) == npoints:
                         hc1_isallzero = True
                            
                    if len(df_tmp[df_tmp['hitcount2']==0]) == npoints:
                         hc1_isallzero = True   
